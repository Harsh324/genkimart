...
export const CartProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const add = useCallback((product: ProductItem, quantity = 1, opts?: AddOpts) => {
    ...
    persistSet(prev => {
      ...
      return [...prev, { product, quantity: Math.max(1, Math.floor(quantity || 1)) }];
    });
    if (isLoaded && !opts?.silent) toast.success('Added to cart', { containerId: 'app-toaster' });
  }, [persistSet, isLoaded]);
  ...
  const checkout = useCallback(async () => {
    if (!items.length) {
      toast.error("Your cart is empty", { containerId: "app-toaster" });
      return;
    }

    if (loading) {
      toast.info("Checking login status...", { containerId: "app-toaster" });
      return;
    }

    if (!user) {
      toast.info("Please login to proceed to checkout", { containerId: "app-toaster" });
      localStorage.setItem("postLoginRedirect", "/checkout");
      window.location.href = "/login";
      return;
    }
    console.log("here at line 527")
    try {
      // Format items for backend
      const payload = items.map(i => ({
        product: i.product.id, // backend expects product UUID
        quantity: i.quantity,
      }));
      console.log("Print at line 534")

      await saveCartToDb(payload);

      toast.success("Cart saved! Redirecting to checkout...", { containerId: "app-toaster" });

      // Clear local cart if needed
      // clear({ silent: true });

      // Redirect (Next.js example)
      window.location.href = "/checkout";
    } catch (err) {
      console.error(err);
      toast.error("Checkout failed", { containerId: "app-toaster" });
    }
  }, [items, clear, user, loading]);
  ...
  const value = useMemo<CartContextProps>(() => ({
    ...
    isLoaded,
    checkout,
  }), [items, add, remove, clear, updateQuantity, isLoaded, checkout]);
